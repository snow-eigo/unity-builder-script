...
namespace UnityBuilderScript.Versioning
{
    public static class Git
    {
        ...
        {
            var version = Run(@"tag --points-at HEAD | grep v[0-9]*");
        ...
        {
            var numberOfCommitsAsString = Run(@"git rev-list --count HEAD");
        ...
        static string Run(string arguments)
        {
            using (var process = new System.Diagnostics.Process())
            {
                ...
                var exitCode = process.Run(application, arguments, workingDirectory, out string output, out string errors);
